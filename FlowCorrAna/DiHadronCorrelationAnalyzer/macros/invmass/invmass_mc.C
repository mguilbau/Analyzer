...
//  TFile* f = new TFile("/net/hisrv0001/home/davidlw/OSG_CMSSW/CMSSW_5_3_16_correlations/src/FlowCorrAna/DiHadronCorrelationAnalyzer/cfg/invmass.root");
  TFile* f = new TFile("/net/hisrv0001/home/davidlw/scratch1/DiHadronCorrelations/outputs_312/HijingReco_Minbias_5TeV/merged/invmassk0spi_all_diff.root");
  ...
  TH2D* hgen = f->Get("invmass_gen_ana/dNdetadptall_trg");
  TH1D* hgen_pt = (TH1D*)hgen->ProjectionY("hgen_pt",hgen->GetXaxis()->FindBin(-2.4),hgen->GetXaxis()->FindBin(2.4),"e");
...
//    fit[i]->SetParLimits(2,0.007,0.04);
    fit[i]->SetParLimits(1,0.387,0.396);
    fit[i]->SetParLimits(2,0.004,0.012);
    fit[i]->SetParLimits(4,0.03,0.08);

/*
    fit_gaus[i] = new TF1(Form("fit_gaus_%d",i),"[0]/sqrt(2*TMath::Pi())/[2]*TMath::Exp(-0.5*(x-[1])*(x-[1])/[2]/[2])+[3]*TMath::Exp(-0.5*(x-[1])*(x-[1])/[4]/[4])",0.6-0.37,1.2-0.65);
    fit_pol[i] = new TF1(Form("fit_pol_%d",i),"[0]+[1]*x+[2]*x*x+[3]*x*x*x+[4]*x*x*x*x",0.6-0.37,1.2-0.65);
    fit[i] = new TF1(Form("fit_%d",i),Form("fit_gaus_%d+fit_pol_%d",i,i),0.6-0.37,1.2-0.65);
    fit[i]->SetParameters(2000,0.39,0.02,0.04,0,10000,-10000,0,0,0,10000);
    fit[i]->FixParameter(3,0);
    fit[i]->SetParLimits(0,0,10000000);
    fit[i]->SetParLimits(1,0.39,0.40);
    fit[i]->SetParLimits(2,0.015,0.027);
*/
    ...
    yield[i]=fit[i]->GetParameter(0)/width/nevents/ptmean[i]/2/PI/4.8;
    yield_err[i]=fit[i]->GetParError(0)/width/nevents/ptmean[i]/2/PI/4.8;

    ibinmin = hgen_pt->GetXaxis()->FindBin(ptbins[i]);
    ibinmax = hgen_pt->GetXaxis()->FindBin(ptbins[i+1]-0.00001);
    yield_gen[i]  = hgen_pt->IntegralAndError(ibinmin,ibinmax,yield_gen_err[i]);
    yield_gen[i] = yield_gen[i]/width/nevents/ptmean[i]/2/PI/4.8;
    yield_gen_err[i] = yield_gen_err[i]/width/nevents/ptmean[i]/2/PI/4.8;

    eff[i] = yield[i]/yield_gen[i];
    eff_err[i] = eff[i]*sqrt(yield_gen_err[i]*yield_gen_err[i]/yield_gen[i]/yield_gen[i]+yield_err[i]*yield_err[i]/yield[i]/yield[i]);
/*
...
*/
  ...
  TGraphErrors* gr_gen = new TGraphErrors(9,ptmean,yield_gen,ptmean_err,yield_gen_err);
  TGraphErrors* gr_eff = new TGraphErrors(9,ptmean,eff,ptmean_err,eff_err);
  ...
  {
    c->cd(i+1);
    hsignal_1D[i]->SetAxisRange(0.25,0.8,"X");
...
//    hback_1D[i]->Draw("PESAME");
  ...
  {
    c1->cd(i+1);
    hsignal_sub_1D[i]->SetAxisRange(0.25,0.8,"X");
...
//    xframe[i]->Draw("same");
  ...
  c3->SetLogy();
  TH2D* htmp1 = new TH2D("htmp1",";p_{T} (GeV/c);Efficiency",10000,0,10,10000,0.000001,1);
  htmp1->Draw();
