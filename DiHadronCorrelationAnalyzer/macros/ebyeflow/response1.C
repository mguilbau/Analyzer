...
{
/*
  TFile* f = new TFile("/net/hisrv0001/home/davidlw/scratch1/DiHadronCorrelations/outputs_312/PAData_Minbias_5TeV/merged/ebyeflow_n220260_pttrg033_rereco_eff1_v3.root");
...
*/
  ...
  TH1D* hvx = (TH1D*)hvxy->ProjectionX("hvx",-1,-1,"e");
  TH1D* hvy = (TH1D*)hvxy->ProjectionY("hvy",-1,-1,"e");
...
//  TF1* fity = new TF1("fity","[0]*TMath::Exp(-0.5*x*x/[1]/[1])",-0.6,0.6);
  ...
  TH2D* hresx = new TH2D("resx",";v_{2,x}^{obs};v_{2,x}^{true}",200,-1.0,1.0,200,-1.0,1.0);
  TH2D* hresy = new TH2D("resy",";v_{2,y}^{obs};v_{2,y}^{true}",200,-1.0,1.0,200,-1.0,1.0);
...
*/
  for(int i=1;i<=hresx->GetNbinsY();i++)
  {
    double vxtrue = hresx->GetYaxis()->GetBinCenter(i);
    for(int j=1;j<=hresx->GetNbinsX();j++)
    {
      double vxmeas = hresx->GetXaxis()->GetBinCenter(j);
      hresx->SetBinContent(j,i,fitx->Eval(vxmeas-vxtrue)); 
    }
  }

  for(int i=1;i<=hresy->GetNbinsY();i++)
  {
    double vytrue = hresy->GetYaxis()->GetBinCenter(i);
    for(int j=1;j<=hresy->GetNbinsX();j++)
    {
      double vymeas = hresy->GetXaxis()->GetBinCenter(j);
      hresy->SetBinContent(j,i,fity->Eval(vymeas-vytrue));
    }
  }
  ...
  TFile* fout = new TFile("ebyeflow_hist.root","recreate");
  hresx->Write("resx");
  hresy->Write("resy");
  hvxy->Write("meas");
  fout->Close();
